...
export const fetchArticle=()=>(dispatch)=>{
	...
	.then((r)=>{
		...
		const commentAuthors = r.articles.reduce((o,article)=>{
			article.comments.map((c)=>c.author).forEach((author)=>o.push(author))
			return o
		},[] )
		const avatars={}
		const ArticleAuthors = r.articles.reduce((o,article)=>{
			o.push(article.author)
			return o
		},[])
		const authors = new Set(commentAuthors.concat(ArticleAuthors))
